...
Try
{
    if (Get-Module LogTools -ListAvailable -Verbose:$false)
    {   
        ...
        # This line sets the $PSBoundParameters['Verbose'] value to a hard false (instead of null) if the Verbose is not a bound parameter. 
        # That is, that the Verbose parameter is not set when the script is run. 
        # This parameter will be passed to the VerboseMode parameter when running Enable-LogfileHooking below.
        # By default, $PSBoundParameters['Verbose'] is set to $true if the switch is selected when running the script, but it is 
        # $null if the option is not selected. The statement below sets the parameter to $false if it is not selected. This is because 
        # if we passed a $null value to a switch parameter (as we will do below), that will result in setting the switch to $true
        ...
        #   -VerboseMode tells the module to write all Write-Verbose statements to the screen as well as to the log. 
        #       The method of setting this parameter below is used in conjunction with . 
        #       You could also pass a direct boolean value ($true or $false) to the Enable-LogfileHooking -VerboseMode parameter
        #       or use the switch with no value to set it as true. However, using the method used here, we set the switch to $true or $false
        #       based on whether the -Verbose switch was used when running this script, and that determines the behavior for Write-Verbose  
        #       throughout this script and any cmdlets that it calls.

        Enable-LogFileHooking -Logfile $LogFile -ThrowExceptionOnWriteError -VerboseMode:$PSBoundParameters['Verbose']
