...
function New-LogDirectory ()
{
    ...
    Try 
    {
        ...
        if (($Name -like "*:\*") -or ($Name -like "\\*"))
        {
            if ([bool]$Path)
            {
                Write-Verbose "$VerboseFnPrefix An explicit path format was provided for the 'Name' argument. The 'Path' argument will be ignored."
            }
...
function Enable-LogFileHooking
{
    ...
    .PARAMETER VerboseMode
        The VerboseMode switch parameter indicates whether or not all Write-Verbose statements should be displayed on-screen, or merely logged to the logfile. 

        When the VerboseMode switch parameter is set to $true, a global variable called VerboseModeGlobal, which is used by the Write-LogMessage function and directs it write all Write-Verbose statements to the screen. 
    ...
    .EXAMPLE
        PS C:\> $Logfile = New-LogfileName -LogBaseName TestLog -LogDir .\ 
        Enable-LogFileHooking -Logfile $LogFile -VerboseMode -IncludeRunspaceID -ThrowExceptionOnWriteError
    ...
    .EXAMPLE
        PS C:\> $Logfile = New-LogfileName -LogBaseName TestLog -LogDir .\ 
        Enable-LogFileHooking -Logfile $LogFile -VerboseMode -Debug -IncludeRunspaceID -ThrowExceptionOnWriteError
    ...
    .EXAMPLE
        PS C:\> $Logfile = New-LogfileName -LogBaseName TestLog -LogDir .\ 
        Enable-LogFileHooking -Logfile $LogFile -VerboseMode -Debug -IncludeRunspaceID -SupressConsoleOutput
    ...
    Param(
        ...
        [Parameter(Mandatory = $false,Position=2)][switch]$VerboseMode
    )
    ...
    if ($PSBoundParameters['VerboseMode'])  
    {
        Write-Verbose "Verbose mode selected. Enabling output to screen."
